...
export default async function SolarPage(
  ...
  const [plantsRes, sourcesRes, productionRes, locationsRes] = await Promise.all([
    ...
    // Use RPC function specifically to securely bypass 1000 row REST limit when aggregating an entire year or large periods
  ...
  // The RPC natively groups data on Postgres guaranteeing 1 perfectly summed row per plant_code
  const productionByPlant = new Map<string, number>()
  for (const row of productionRes.data ?? []) {
    productionByPlant.set(row.plant_code, Number(row.total_kwh) || 0)
  }
                ...
                plants.map((plant) => (
                  <tr key={plant.plant_code} className="hover:bg-cream-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right">
                      <Link 
                        href={`/solar/${encodeURIComponent(plant.plant_code)}`}
                        className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-forest-100 text-forest-700 text-xs font-bold hover:bg-forest-200 transition-colors cursor-pointer"
                      >
                        +
                      </Link>
                    </td>
